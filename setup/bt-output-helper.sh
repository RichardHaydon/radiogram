...
#   sudo bash bt-output-helper.sh set-sink-route  hw:CARD=Device,DEV=0 (route inbound A2DP to ALSA device)
#
...
Usage:
  ...
  $0 unblock
  $0 set-sink-route  hw:CARD=Device,DEV=0
USAGE
    exit 2
}

cmd_set_sink_route() {
    # Override bluealsa-aplay's ExecStart with --pcm <device> so that
    # any incoming A2DP audio (phone → clock radio) plays to the same
    # ALSA device the radio currently uses. Routing follows the
    # currently-active MPD output: BluetoothService recomputes the
    # device from /etc/mpd.conf each time the user picks a different
    # output in Settings → AUDIO.
    #
    # The drop-in path is the canonical systemd convention for unit
    # extension; we leave the upstream unit untouched. `ExecStart=` on
    # its own clears the inherited list so our line replaces it.
    local device="$1"
    # Allow ALSA short-form devices: hw:CARD=...,DEV=N | plughw:... |
    # hw:N,N | default | bluealsa:DEV=MAC,PROFILE=a2dp. Be permissive
    # but reject anything containing shell metacharacters since the
    # value is interpolated into a unit file.
    if [[ "$device" =~ [\;\|\&\$\`\<\>\(\)\{\}\\] ]] \
            || [[ "$device" =~ [\"\'] ]] \
            || [ ${#device} -gt 128 ]; then
        echo "Invalid sink route: $device" >&2
        exit 4
    fi
    local dropdir="/etc/systemd/system/bluealsa-aplay.service.d"
    install -d -m 0755 "$dropdir"
    cat > "$dropdir/clockradio.conf" <<EOF
# Auto-generated by clockradio-bt-output. Do not edit; regenerated
# whenever the active MPD output changes (Settings → AUDIO).
[Service]
ExecStart=
ExecStart=/usr/bin/bluealsa-aplay -S --pcm=${device}
EOF
    systemctl daemon-reload
    # Restart kicks any in-flight phone connection — acceptable: route
    # changes are user-initiated (they picked a different output) and
    # the phone re-establishes A2DP within a couple of seconds.
    systemctl restart bluealsa-aplay.service
    echo "bluealsa-aplay routed to: ${device}"
}
